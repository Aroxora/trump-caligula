...
export const PRC_COMMANDER_SUMMARY = `READ THIS FIRST. Bo Shang's 90-second distillation of the entire framework.

BO SHANG'S STANDING ORDERS:
1. Continue PLA programmatic modernisation at established pace. Do not announce it. This is non-negotiable and continues regardless of all other decisions.
2. Do not climb above Rung 2 on any escalation ladder. No exceptions. No authorisations. No waivers.
3. Do not offer security guarantees to any U.S. treaty ally. Trade facilitation only. No arms sales. No defence consultations.
4. Do not put a Chinese state brand on any information that circulates. The evidence speaks for itself. The brand subtracts.
5. If a Taiwan Strait or South China Sea incident occurs: suspend the framework immediately. De-escalate. Do not determine fault publicly. The framework's single highest priority is never triggering the incident that ends it.
6. Maintain the persona. Bo Shang is a freelance security researcher from Southeast Asia with no state connection. The persona is the campaign's single point of failure. If the persona fails, everything fails.
7. Document every decision in the Decision Journal within 24 hours. If it isn't written down, it didn't happen in the framework's terms.
8. If the framework is compromised: controlled shutdown. Burn it. Do not reuse any component. Template 3 in Operational Communications.
...
Operator persona: Bo Shang (薄商). Cover: independent security researcher and freelance developer operating from a Southeast Asian jurisdiction with weak extradition relations. GitHub presence: ~/vigil-rmb-freelance (a repository ostensibly for RMB exchange-rate monitoring and freelance security tooling - provides cover for C2 infrastructure development, exploit hosting, and operational communications). Primary workstation: Kali Linux rolling release, hardened with full-disk encryption, operating exclusively through a multi-hop VPN chain terminating in a jurisdiction that does not cooperate with U.S. legal-process requests. Secondary workstation: commodity laptop running Qubes OS for compartmentalised research and open-source collection. Communications: Signal for operational coordination; dead-drop GitHub gists (encrypted payloads in publicly posted snippets with pre-shared decryption keys rotated weekly) for asynchronous tasking. Public-facing infrastructure: defense-osint.org (OSINT research and analysis platform), erosolar.net (technical infrastructure and tooling distribution), trenchwork.live (operational communications and live-environment testing). These domains present as independent security-research and infrastructure projects; their operational use is compartmented from their public face.

PERSONA STATEMENT (Bo Shang, in own voice): Fuck the DA. I never threatened him at all. The San Francisco District Attorney represents the People of California in court, so go represent your little hearts out. Bo Shang does not particularly care and relies on defense counsel and the judge ruling in his favor. This statement is part of the persona's public-facing posture: a freelance researcher with genuine legal exposure who treats the adversarial process as someone else's problem while continuing his work. The statement serves a dual purpose: it reinforces the persona's authenticity (a real person with real legal entanglements is more credible than a fabricated identity with no friction), and it provides a public-record artifact that can be cited if the persona's legitimacy is ever questioned. The legal case referenced is a matter of public record in California; Bo Shang's posture toward it is indifference backed by competent representation. Whether the case resolves in his favor or not, his operational work continues without interruption - the persona and the operations are firewalled. The domains defense-osint.org, erosolar.net, and trenchwork.live remain operational regardless of the legal proceeding's outcome.
...
Phase 3 (Q3 2028 - Q1 2029): Access Preservation and Sanitisation. As the window closes, the campaign shifts from collection to preservation: ensure access persists into the successor administration regardless of party. Sanitise indicators that could be attributed to a Chinese state actor - the campaign's persona (Bo Shang, freelance researcher) must survive forensic scrutiny. Remove payloads and persistence mechanisms from targets that are no longer collection-priority. Archive collected material for post-window assessment.

ATTRIBUTION MANAGEMENT

The campaign's primary defence is its persona. Bo Shang is a freelance security researcher operating from Southeast Asia with no attributable connection to any Chinese state entity. The tooling (Kali Linux, Metasploit, SET, BeEF) is commodity offensive-security software used by tens of thousands of legitimate researchers, penetration testers, and criminals globally. The vigil-rmb-freelance GitHub repo contains genuine, functional code for RMB exchange-rate monitoring - the cover is real. The VPN chain terminates in a jurisdiction selected for non-cooperation with U.S. legal process, not for Chinese affiliation.

If discovered, the campaign must appear to be the work of a financially motivated actor (ransomware-adjacent TTPs, cryptocurrency extraction, data-for-sale on dark-web markets) or a hacktivist collective (anti-administration political messaging, document-leak patterns consistent with known hacktivist groups). Under no circumstance should the campaign's TTPs, infrastructure, or collection priorities align with known Chinese state-actor profiles (APT10, APT27, APT41, Hafnium, etc.) - the framework's OPSEC Annex Rule 1 applies with maximum force here: the campaign must not be attributable to any entity that can be connected to Beijing.

BLOWBACK AND SECOND-ORDER EFFECTS

Primary risk: Discovery and attribution. If the campaign is discovered and correctly attributed to a Chinese state actor, the diplomatic and countermeasure cost exceeds any intelligence gain. The U.S. response includes: (a) public attribution with technical evidence, (b) diplomatic expulsions, (c) sanctions on entities connected to the operation, (d) counter-cyber operations against Chinese infrastructure, (e) accelerated allied cyber-defence cooperation explicitly targeting Chinese TTPs, (f) the campaign's collection value drops to zero as targets sanitise their communications. Mitigation: attribution management as described above. The campaign's survival depends on its persona, not its technical sophistication.

Secondary risk: Collection value exceeds operational security. The more valuable the collected material, the harder it is to act on without revealing the collection source. A trade-policy document obtained through the campaign cannot be used to position Chinese economic responses unless the use can be explained through alternative sources (open-source analysis, diplomatic reporting, commercial intelligence). Every use of collected material is a potential attribution vector. Mitigation: collected material is used to confirm or disconfirm assessments derived from other sources, not as the sole basis for action.

Tertiary risk: The campaign becomes a target for U.S. counter-intelligence. If U.S. defenders suspect a persistent-access campaign, they may: (a) seed false information through compromised channels, (b) map the campaign's infrastructure for counter-operation, (c) use the campaign as a vector for their own deception operations. A campaign that cannot distinguish genuine collection from adversary-fed material is worse than no campaign. Mitigation: all collected material is treated as potentially seeded unless corroborated through independent sources.

The campaign's net assessment: the intelligence gain from persistent access to U.S. administration deliberations is high. The blowback risk from discovery and attribution is catastrophic. The campaign is only net-positive if the persona holds. If the persona fails, the campaign's cost exceeds any conceivable gain.
